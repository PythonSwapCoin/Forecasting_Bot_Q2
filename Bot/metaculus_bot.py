...
async def get_binary_gpt_prediction(
    ...
    fine_print = getattr(question_details, "fine_print", "")
...
class ProxiedForecaster(ForecastBot):
    def __init__(self, llms=None, **kwargs):
        super().__init__(llms=llms, **kwargs)
        self.llms = llms or {}
        self.name = "ProxiedForecaster"

    async def _run_forecast_on_binary(self, question_dict: dict, research) -> tuple[float, str]:
        return await get_binary_gpt_prediction(question_dict, num_runs=5)

    async def _run_forecast_on_numeric(self, question_dict: dict, research) -> tuple[list[float], str]:
        return await get_numeric_gpt_prediction(question_dict, num_runs=5)

    async def _run_forecast_on_multiple_choice(self, question_dict: dict, research) -> tuple[dict[str, float], str]:
        return await get_multiple_choice_gpt_prediction(question_dict, num_runs=5)

    async def run_research(self, question):
        return run_research(question)

    def _llm_config_defaults(self):
        return {"default": "gpt-4"}
